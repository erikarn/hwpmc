...
open ($p, "pmcstat -s CPU_CLK_UNHALTED.THREAD_P -s DTLB_LOAD_MISSES.STLB_HIT -s DTLB_LOAD_MISSES.WALK_DURATION -w 1 2>&1 |") || die "Couldn't run hwpmc: $!\n";
...
#	printf "\n";
	...
	for (my $i = 0; $i < scalar @h; $i++) {
		...
		my ($clk_val) = $d{$h[$i]};
		my ($stlb_hit_val) = $d{"s/" . $a[1] . "/DTLB_LOAD_MISSES.STLB_HIT"};
		my ($stlb_walk_val) = $d{"s/" . $a[1] . "/DTLB_LOAD_MISSES.WALK_DURATION"};
